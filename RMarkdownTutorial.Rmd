...
This is an R Markdown document (`.Rmd`). Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown I recommend *Bookdown* by @bookdown2016 <https://bookdown.org/yihui/rmarkdown/markdown-syntax.html>.  

Above this text there is some code surrounded by "---" which sets up the document. There, you can set defaults and formatting for the document output. I've set it to html, but it can be changed to pdf_document, word_document
...
You can also use math environments inside dollar signs like this cool matrix:  
...
# Chunks
Chunks are where the magic happens. Ie. where you import data, mess with it, code, calculate, make figures, etc. You can insert an R code chunk either using the RStudio toolbar (the Insert button) or the keyboard shortcut Ctrl + Alt + I (Cmd + Option + I on macOS).  
```{r setup, echo = FALSE}
...
```{r Figures, fig.cap= "Figure 1. Fish Condition by Outmigration Week ", message = FALSE, include = TRUE, out.width= '50%',  fig.show ='hold', warning = FALSE}
...
If you want to include a graphic or figure that is not generated from R code, like one you made in Inkscape after Colin's rad workshop, you may use the knitr::include_graphics() function. Just add in the path or url to your image, and remove the hashtag comment in the chunk below. Here's a cute goat gif in the meantime...  

![](https://media.giphy.com/media/TGakgWdFTT1Sg/giphy.gif)
```{r, echo = FALSE, message = FALSE, include = TRUE, out.width='25%', fig.align='center', fig.cap='...'}
...
### Basic
```{r Tables, message = FALSE, include = TRUE, results = 'asis'}
...
### Summary
```{r Summary Stats Tables, message = FALSE, include = TRUE, results = 'asis'}
#make sure results = 'asis' is there
...
# Citing R packages
Here's an easy chunk that will create a bib file of all the packages you have loaded for this R script. This is super useful for your methods when you're running analysis and making figures. This will create a new file "packages.bib" that you can import to your citation manager (Mendeley) or use as a bibliography directly in markdown when writing a manuscript.  
```{r packages}
